...
impl TailwindToml {
    ...
    pub fn to_js(&self) -> String {
        let content = self.content.as_ref().map(|c| serde_json::to_string(c).unwrap()).unwrap_or_else(|| "[]".to_string());
        let theme = self.theme.as_ref().map(|t| serde_json::to_string_pretty(t).unwrap()).unwrap_or_else(|| "{}".to_string());
...
pub fn ensure_tailwind_config(project_root: &Path, style: super::TailwindStyle) -> Result<Option<std::path::PathBuf>> {
    ...
    if matches!(style, super::TailwindStyle::Toml) || (matches!(style, super::TailwindStyle::Auto) && toml_path.exists()) {
